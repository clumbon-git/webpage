...
$stmt = $pdo -> prepare('
	SELECT COUNT(*) FROM `members_home`
	WHERE `id_member_home` = :id AND `change_pass_key` = :key
');
...
if($t_count != 1 && $P_gimmick_type != "sign_up"){
	...
	F_workLog($t_id, 1, __FILE__, 0, $P_gimmick_type, $t_key);
...
';
	...
	F_workLog($t_id, 1, __FILE__, 0, $P_gimmick_type);
...
<title><?php echo $G_npo_items['name_npo']; ?>会員パスワード登録</title>
