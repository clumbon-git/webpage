...
isset($_POST["childid"]) && $P_childid = $_POST["childid"];
...
if($P_gimmick_type == "home_up"){
	...
	$P_gimmick_type = "details";
}elseif($P_gimmick_type == "childPicUp"){
	//児童キャラ画像更新
	if(isset($_FILES['cpic_' . $P_childid]) && is_uploaded_file($_FILES['cpic_' . $P_childid]['tmp_name'])){
		$flg_upLinkPic = false;
		//画像格納
		$name_chara = 'c_' . $P_childid;
		$name_path = './pic';
		list($flg_up, $mess_up) = F_picUp('cpic_' . $P_childid, $name_path, $name_chara, $GV_child_pic_x, $GV_child_pic_y, 1, 0);
		if($flg_up){
			$flg_upLinkPic = true;
			$filename_chara = $mess_up;

			if($flg_upLinkPic){
				//児童キャラ画像格納成功
				//画像ファイル名DB登録
				$stmt = $pdo -> prepare('
	UPDATE `members_child` SET `chara_pic` = :link WHERE `id_child` = :id
				');
				$stmt -> bindValue(':id', $P_childid, PDO::PARAM_INT);
				$stmt -> bindValue(':link', $filename_chara, PDO::PARAM_STR);
				$stmt -> execute();

				$G_workmes .= '<span class="col_blue">ファイル情報をDBに記録しました</span><br>';
			}

			$G_workmes .= '<span class="col_blue">児童キャラ画像を登録しました</span><br>';
		}else{
			$G_workmes .= '<span class="col_red">(児童キャラ画像)' . $mess_up . '</span>';
		}

	}else{
		$G_workmes = '<span class="col_red">児童キャラ画像更新エラー、画像のファイルサイズを確認してください</span>';
	}
	$P_gimmick_type = "details";
}
...
<script src="./js/memberdetails.js?<?php echo date("Ymd-Hi"); ?>"></script>
			...
			<form name="fm_parents" action="" method="post">
				...
				echo '<tr>
					...
					<tr>
						<td>保護者追加</td>
						<td>
氏<input type="text" value="<?php echo $P_name1; ?>" size="20" name="name1_new" placeholder="名字">
名<input type="text" value="<?php echo $P_name2; ?>" size="20" name="name2_new" placeholder="名前"><br>
　<input type="text" value="<?php echo $P_kana1; ?>" size="20" name="kana1_new" placeholder="ミョウジ">
　<input type="text" value="<?php echo $P_kana2; ?>" size="20" name="kana2_new" placeholder="ナマエ"><br>
...
<br>
			<form name="fm_child" action="" method="post" enctype="multipart/form-data">
				<table class="config">
...
<?php
	...
	foreach($hall_names as $val){
		if($val['id_hall'] != $G_home_d['fk_id_hall']){
			continue;
		}
...
function F_makedisp_child($f_c){
	...
	//児童キャラ画像登録
	$t_chara_pic = '<span class="col_red">未登録</span><br>';	//登録済み画像
	if($f_c['chara_pic'] && file_exists('./pic/' . $f_c['chara_pic'])){
		$t_chara_pic = '<img border="1" src="./pic/' . $f_c['chara_pic'] . '" class="child_pic"><br>';
	}
	$t_chara_up = '<b>入退室目印画像登録</b><br>好きなキャラなどの画像を使ってください。<br>本人の写真は使用しないでください。<br>使用できる画像ファイルは2M以下です。<br>';
	$t_chara_up .= $t_chara_pic;
	$t_chara_up .= '<input type="file" name="cpic_' . $f_c['id_child'] . '" accept=".jpg, .jpeg, .png, .gif"><input type="button" value="目印画像更新" onclick="f_childPicUp(this.form, ' . $f_c['id_child'] . ');">';
...
$ret_tex .= '入所日:' . $f_c['enterhall_date'] . '<br>';
$ret_tex .= '<input type= "hidden" name="enth_' . $f_c['id_child'] . '" value="' . $f_c['enterhall_date'] . '">';
$ret_tex .= '<textarea rows="5" cols="70" name="nc_' . $f_c['id_child'] . '" placeholder="食物アレルギー等メモ">' . $f_c['note'] . '</textarea><br>';
$ret_tex .= '　<input type="button" value="　編集内容更新　" onClick="f_edit_c(this.form, ' . $f_c['id_child'] . ');"><br><br>';
$ret_tex .= $t_chara_up;
...
<tr>
	...
	<td>'
		...
		'　メモ<input type="text" name="pnone_' . $f_t['id_member_phone'] . '" size="30" maxlength="30" value="' . $f_t['note'] . '">';
			$ret_tex .= '<input type="button" value="メモ更新" onClick="f_phone_n(' . $f_t['id_member_phone'] . ', \'pnone_' . $f_t['id_member_phone'] . '\');"><br>';
			$ret_tex .= '※緊急時にかける順番をメモに記入してください';
	$ret_tex .= '</td>
</tr>';
	return $ret_tex;
}
