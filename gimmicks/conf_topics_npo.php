...
//操作対象館idセット
$P_hall_id = 0;  //編集対象 -1=指定無し 0=NPO 1<=各館id
...
if($P_gimmick_type == "disp_change"){
	...
	$G_workmes = '<span class="col_blue">表示を切り替えました</span>';
		...
		F_workLog($G_login['id'], $G_login['kind'], __FILE__, $G_gimmickno, $P_gimmick_type, $P_topic_id);
	...
	$G_workmes = '<span class="col_blue">指定トピックを変更しました</span>';
		...
		F_workLog($G_login['id'], $G_login['kind'], __FILE__, $G_gimmickno, $P_gimmick_type, $P_topic_id, $P_title, $P_text);
	...
	$G_workmes = '<span class="col_blue">新規トピックを追加しました</span>';
		...
		F_workLog($G_login['id'], $G_login['kind'], __FILE__, $G_gimmickno, $P_gimmick_type, $P_hall_id, $P_title, $P_text);
...
VALUES(:id, :folder, :pic_main, :pic_thum)
				...
				F_workLog($G_login['id'], $G_login['kind'], __FILE__, $G_gimmickno, $P_gimmick_type, $P_topic_id, $folder_ym, $filename_mainComp, $filename_thumComp);
...
WHERE `id_pic` = :id
	...
	$G_workmes = '<span class="col_blue">指定画像を削除しました</span>';
		...
		F_workLog($G_login['id'], $G_login['kind'], __FILE__, $G_gimmickno, $P_gimmick_type, $P_pic_id);
...
<script src="./js/conf_topics_npo.js"></script>
