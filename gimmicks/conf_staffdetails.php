...
if($P_gimmick_type == "up"){
	...
	if($t_datecheck === false){
		...
		$G_workmes .= '<span class="col_blue">※指定職員を退職処理しました</span><br>';
			...
			F_workLog($G_login['id'], $G_login['kind'], __FILE__, $G_gimmickno, $P_gimmick_type, $P_staffid, $P_lev_hall);
...
<title><?php echo $G_npo_items['name_npo']; ?>職員情報更新</title>
</head>
<body>
<div id="pagebody">
...
<!-- 入力 -->
		...
		<p>
			...
			<a href = "./conf_staffmanagement.php" target = "_self">・職員一覧、編集トップへ戻る</a><br>
		</p>
		<hr>
<?php
echo $G_workmes;
...
自己紹介:<textarea rows="5" cols="70" name= "comment" placeholder="ホームページ表示指導員からテキスト"><?php echo $P_comment; ?></textarea>
			...
			?>
			<hr>
			<form name="fm_lev" action="" method="post" onsubmit="return false;">
				<input type="text" name="lev_hall" size="10" value="<?php echo $P_lev_hall; ?>" placeholder="退職日"><span class="t80per">(例 2019-4-1 2019/4/1 20190401)</span>
				<input type="button" value="退職" onClick="if(window.confirm('退職処理を実行します')){submit();}">
				<input type="hidden" name="gimmick_type" value="lev">
				<input type="hidden" name="staffid" value=<?php echo $P_staffid; ?>>
			</form>
			<?php
			}
